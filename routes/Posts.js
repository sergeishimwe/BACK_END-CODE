  ...
  blogDb.findOneAndUpdate(
    ...
    function (error, success) {
      if (error) {
        ...
        res
          .status(201)
          .send({ message: `comment has been committed to the blog!!` });
      }
    }
  );
});
  ...
  blogDb
    ...
    .then((data) => {
      if (!data) {
        res
          .status(403)
          .send({ mesage: `Content to delete already doesn't exist` });
      } else {
        res.status(202).send({
    ...
    router.put("/:id", requireAuth, async (req, res) => {
      ...
      blogDb
        ...
        .catch((err) => {
          res
            .status(400)
            .send({ message: "Not blog found with the provided ID" });
        });
    });
